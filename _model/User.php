...
class User
{
     ...
     * @param $specialized
     */
    public function __construct($userID, $username, $password, $firstName, $lastName, $email, $role, $address, $price, $subjects_ids, $description, $birthday, $sex, $phone, $experience, $experience_type, $degree, $graduation_school, $specialized, $schedule)
    {
        $this->userID = $userID;
    ...
    public static function getAll()
    {
        ...
        if (mysqli_num_rows($rs) > 0) {
            while ($row = mysqli_fetch_object($rs)) {
                $user = new user($row->userID, $row->username, $row->password, $row->firstName, $row->lastName, $row->email, $row->role, $row->address, $row->price, $row->subjects_ids, $row->description, $row->birthday, $row->sex, $row->phone, $row->experience, $row->experience_type, $row->degree, $row->graduation_school, $row->specialized, $row->schedule);
                $rows[] = $user;
            }
        }
